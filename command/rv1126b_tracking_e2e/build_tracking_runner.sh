#!/usr/bin/env bash
# Build the tracking-mode E2E runner for RV1126B's ARM EABI5 hard-float ABI.
set -euo pipefail
...
output_dir=${1:-"$repository/build/rv1126b-tracking-e2e"}
...
"$compiler-g++" -std=c++14 -O2 -Wall -Wextra -Werror \
    -I"$sdk_dir/include" \
    "$script_dir/tracking_e2e_runner.cpp" -L"$sdk_dir/lib" -L"$rknn_library_directory" -lInspireFace -Wl,--no-as-needed -lrknnrt -Wl,--as-needed -ldl \
    -Wl,-rpath,'$ORIGIN' -o "$output_dir/tracking_e2e_runner"

cp "$sdk_dir/lib/libInspireFace.so" "$rknn_library" "$output_dir/"
for candidate in "$output_dir/tracking_e2e_runner" "$output_dir/libInspireFace.so" "$output_dir/librknnrt.so"; do
    file "$candidate"
    check_armhf "$candidate"
done
"$compiler-readelf" -d "$output_dir/tracking_e2e_runner" | grep -q 'Shared library: \[libInspireFace.so\]'
"$compiler-readelf" -d "$output_dir/tracking_e2e_runner" | grep -q 'Shared library: \[librknnrt.so\]'
