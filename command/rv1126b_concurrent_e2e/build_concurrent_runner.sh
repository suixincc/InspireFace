...
output_dir=${1:-"$repository/build/rv1126b-concurrent-e2e"}
...
mkdir -p "$output_dir"
"$compiler-g++" -std=c++14 -O2 -Wall -Wextra -Werror -pthread \
    -I"$sdk_dir/include" \
    "$script_dir/concurrent_e2e_runner.cpp" -L"$sdk_dir/lib" -L"$(dirname -- "$rknn_library")" -lInspireFace -Wl,--no-as-needed -lrknnrt -Wl,--as-needed -ldl -lpthread \
    -Wl,-rpath,'$ORIGIN' -o "$output_dir/concurrent_e2e_runner"
cp "$sdk_dir/lib/libInspireFace.so" "$rknn_library" "$output_dir/"
"$compiler-readelf" -d "$output_dir/concurrent_e2e_runner" | grep -q 'Shared library: \[libInspireFace.so\]'
