#!/usr/bin/env bash
# Build the pipeline capability matrix runner for RV1126B ARMHF.
set -euo pipefail
...
output_dir=${1:-"$repository/build/rv1126b-pipeline-matrix"}
...
"$compiler-g++" -std=c++14 -O2 -Wall -Wextra -Werror \
    -I"$sdk_dir/include" \
    "$script_dir/pipeline_matrix_runner.cpp" -L"$sdk_dir/lib" -L"$rknn_library_directory" -lInspireFace -Wl,--no-as-needed -lrknnrt -Wl,--as-needed -ldl \
    -Wl,-rpath,'$ORIGIN' -o "$output_dir/pipeline_matrix_runner"

cp "$sdk_dir/lib/libInspireFace.so" "$rknn_library" "$output_dir/"
for candidate in "$output_dir/pipeline_matrix_runner" "$output_dir/libInspireFace.so" "$output_dir/librknnrt.so"; do
    file "$candidate"
    check_armhf "$candidate"
done
"$compiler-readelf" -d "$output_dir/pipeline_matrix_runner" | grep -q 'Shared library: \[libInspireFace.so\]'
"$compiler-readelf" -d "$output_dir/pipeline_matrix_runner" | grep -q 'Shared library: \[librknnrt.so\]'
